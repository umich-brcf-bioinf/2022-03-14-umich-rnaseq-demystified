...
> Hint:
> 
> The command `rowVars(assay(rld))` will return the row variance values, the command `base::order` will return an index of the input re-arranged into accending or decending order, and the `head` command can be used to select the firstor last part of several data types. 
> 
...
```{r TestSampleHeatmap}
topVarGenes <- head(base::order(rowVars(assay(rld)), decreasing=TRUE), 50)
```
</details>

<br>

- Next, create an object that can store the sample annotations (from the full DESeq2 object `dds`)
<br>
```{r GenerateLabels}
...
- Next, use `pheatmap` to plot the selected `topVarGenes` from `rld`, including using the `annotation_col` to show the column annotations stored in `df`. Scale by row to enhance any clusters of genes with similar expression across samples.  
...
<details>
    <summary>*Click for solution*</summary>
```{r TestPheatmap}
p <- pheatmap(assay(rld)[topVarGenes,], show_rownames=FALSE,
